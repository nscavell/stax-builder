...
public class PushbackXMLStreamReader implements XMLStreamReader
{
   ...
   private Queue<PushbackData> pushbackStream = new ArrayDeque<PushbackData>();
   ...
   {
      return (inPushback && currentData != null ? currentData.getText() : streamReader.getElementText());
   }
   ...
   {
      return (inPushback && currentData != null ? currentData.getText() != null : streamReader.hasText());
   }
   ...
   public void mark()
   {
      ...
      if (getEventType() == START_ELEMENT) //nscavell: if we mark but have already read the START_ELEMENT, push the data.
      {
         pushData();
      }
      marked = true;
   ...
   {
      if (pushbackStream.isEmpty()) return;
